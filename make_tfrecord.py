...
flags.DEFINE_string('data_path', '', 'Path to images and xml data')
FLAGS = flags.FLAGS

#dictionary of class labels, modify as needed
LABEL_DICT =  {
    "egg" : 1,
    "hen" : 2
    }
...
def create_tf_example(example):
    ...
    tree = ET.ElementTree(file=os.path.join(FLAGS.data_path,example))
    ...
    with tf.gfile.GFile(os.path.join(FLAGS.data_path,filename), 'rb') as fid:
    ...
    for child in tree.iter('name'):
        for _class in LABEL_DICT.keys():
            if child.text==_class:
                classes.append(LABEL_DICT[_class])
